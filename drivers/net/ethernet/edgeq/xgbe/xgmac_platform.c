			    ...
			    struct plat_xgmacenet_data *plat)
{
	...
	if (plat->ecpri_en && plat->rx_queues_to_use < MIN_RX_QUEUES_WITH_ECPRI)
		plat->rx_queues_to_use = MIN_RX_QUEUES_WITH_ECPRI;
	...
	/* Use phy rate matching by default */
	plat->phy_rate_matching = true;
	...
	for_each_child_of_node(rx_node, q_node) {
		...
		plat->rx_queues_cfg[queue].ch_irq = of_irq_get(q_node, 0);
		if (plat->rx_queues_cfg[queue].ch_irq < 0)
			return plat->rx_queues_cfg[queue].ch_irq;
		...
		if (ia_node) {
			...
			if (cpu >= 0)
				plat->rx_queues_cfg[queue].ch_irq_cpu = cpu;
			else
				plat->rx_queues_cfg[queue].ch_irq_cpu = 1;
		} else {
			/* Default to cpu 1 */
			plat->rx_queues_cfg[queue].ch_irq_cpu = 1;
		}
	...
	if (plat->ecpri_en) {
		if (plat->eth_link == 1 &&
		    plat->tx_queues_to_use < MIN_TX_QUEUES_WITH_ECPRI_L)
			plat->tx_queues_to_use = MIN_TX_QUEUES_WITH_ECPRI_L;
		if (plat->eth_link == 2 &&
		    plat->tx_queues_to_use < MIN_TX_QUEUES_WITH_ECPRI)
			plat->tx_queues_to_use = MIN_TX_QUEUES_WITH_ECPRI;
	}
	...
	for_each_child_of_node(tx_node, q_node) {
		...
		plat->tx_queues_cfg[queue].ch_irq = of_irq_get(q_node, 0);
		if (plat->tx_queues_cfg[queue].ch_irq < 0)
			return plat->tx_queues_cfg[queue].ch_irq;
		...
		if (ia_node) {
			...
			if (cpu >= 0)
				plat->tx_queues_cfg[queue].ch_irq_cpu = cpu;
			else
				plat->tx_queues_cfg[queue].ch_irq_cpu = 1;
		} else {
			/* Default to cpu 1 */
			plat->tx_queues_cfg[queue].ch_irq_cpu = 1;
		}
...
xgmac_probe_config_dt(struct platform_device *pdev, const char **mac)
{
	...
	*mac = of_get_mac_address(np);
	...
	pmd_support = of_property_read_bool(np, "edgeq,pmd-support");
	if (pmd_support) {
		pmd_enabled = 1;
				  ...
				  struct xgmac_resources *xgmac_res)
{
	...
	if (pdev->num_resources > 4) {
		xgmac_res->misc_base = devm_platform_ioremap_resource_byname(pdev, "misc");
		/* misc_base is for first link only */
		err = PTR_ERR_OR_ZERO(xgmac_res->misc_base);
		if (err)
			return err;
		xgmac_res->e56_base = devm_platform_ioremap_resource_byname(pdev, "e56");
		/* e56_base is for first link only for 50G interface */
		err = PTR_ERR_OR_ZERO(xgmac_res->e56_base);
		if (err)
			return err;
		xgmac_res->bss_base = devm_platform_ioremap_resource_byname(pdev, "bss");
		err = PTR_ERR_OR_ZERO(xgmac_res->bss_base);
		if (err)
			return err;
	}
	else {
