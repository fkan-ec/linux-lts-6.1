...
int pmd_enabled = 0;
module_param(pmd_enabled, int, 0644);
MODULE_PARM_DESC(pmd_enabled, "Enable PMD support");
...
/**
 * xgmac_disable_all_queues - Disable all queues
 * @priv: driver private structure
 */
static void xgmac_disable_all_queues(struct xgmac_priv *priv)
{
	...
	for (queue = 0; queue < maxq; queue++) {
		struct xgmac_channel *ch = &priv->channel[queue];

		if (pmd_enabled && queue == 1)
			continue;
...
/**
 * xgmac_enable_all_queues - Enable all queues
 * @priv: driver private structure
 */
static void xgmac_enable_all_queues(struct xgmac_priv *priv)
{
	u32 rx_queues_cnt = priv->plat->rx_queues_to_use;
	u32 tx_queues_cnt = priv->plat->tx_queues_to_use;
	u32 maxq = max(rx_queues_cnt, tx_queues_cnt);
...
/**
 * xgmac_stop_all_queues - Stop all queues
 * @priv: driver private structure
 */
...
/**
 * xgmac_start_all_queues - Start all queues
 * @priv: driver private structure
 */
...
static inline u32 xgmac_tx_avail(struct xgmac_priv *priv, u32 queue)
{
	struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
	u32 avail;
...
/**
 * xgmac_rx_dirty - Get RX queue dirty
 * @priv: driver private structure
 * @queue: RX queue index
 */
static inline u32 xgmac_rx_dirty(struct xgmac_priv *priv, u32 queue)
{
	struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	u32 dirty;
...
static void xgmac_enable_eee_mode(struct xgmac_priv *priv)
{
	...
	/* If eCPRI enabled, eCPRI SS manages eCPRI Tx queue. Skip eee mode */
	if (priv->plat->ecpri_en || pmd_enabled)
		return;
			    ...
			    struct phylink_link_state *state)
{
	...
	/* Check PHY which modes it supports. */
	xgbe_phy_validate(priv, supported, state);
}
				     ...
				     struct phylink_link_state *state)
{
	...
	if (priv->plat->interface == PHY_INTERFACE_MODE_SGMII ||
		...
		xgbe_phy_rtl_get_state(priv, state);
	else
			       ...
			       bool tx_pause, bool rx_pause)
{
	...
	if (priv->plat->phy_rate_matching)
...
static int xgmac_phy_setup(struct xgmac_priv *priv)
{
	...
	xgbe_phy_supported_features(priv);
...
static void xgmac_display_rx_rings(struct xgmac_priv *priv)
{
	...
	/* Display RX rings */
	for (queue = num_ecpri_q; queue < rx_cnt; queue++) {
		struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
...
static void xgmac_display_tx_rings(struct xgmac_priv *priv)
{
	...
	/* Display TX rings */
	for (queue = num_ecpri_q; queue < tx_cnt; queue++) {
		struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
		...
		else if (tx_q->tbs & XGMAC_TBS_AVAIL)
 ...
 */
static void xgmac_clear_rx_descriptors(struct xgmac_priv *priv, u32 queue)
{
	struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	int i;
	...
	for (i = 0; i < DMA_RX_SIZE; i++)
		if (priv->extend_desc)
			xgmac_init_rx_desc(priv, &rx_q->dma_erx[i].basic,
					   priv->use_riwt);
		else
			xgmac_init_rx_desc(priv, &rx_q->dma_rx[i],
					   priv->use_riwt);
}
 ...
 * in case of both basic and extended descriptors are used.
 */
static void xgmac_clear_tx_descriptors(struct xgmac_priv *priv, u32 queue)
{
	struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
	int i;
...
static void xgmac_clear_descriptors(struct xgmac_priv *priv)
{
	...
	/* Clear the RX descriptors */
	for (queue = num_rx_ecpri_q; queue < rx_queue_cnt; queue++) {
		if (pmd_enabled && queue == 1)
			continue;

		xgmac_clear_rx_descriptors(priv, queue);
	}

	/* Clear the TX descriptors */
	for (queue = num_tx_ecpri_q; queue < tx_queue_cnt; queue++) {
		if (pmd_enabled && queue == 1)
			continue;

		xgmac_clear_tx_descriptors(priv, queue);
	}
}
...
{
	struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	struct xgmac_rx_buffer *buf = &rx_q->buf_pool[i];
 ...
 * @i: buffer index.
 */
static void xgmac_free_rx_buffer(struct xgmac_priv *priv, u32 queue, int i)
{
	struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	struct xgmac_rx_buffer *buf = &rx_q->buf_pool[i];
 ...
 * @i: buffer index.
 */
static void xgmac_free_tx_buffer(struct xgmac_priv *priv, u32 queue, int i)
{
	struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
...
static int init_dma_rx_desc_rings(struct net_device *dev, gfp_t flags)
{
	...
	/* The eCPRI queue points to host buffer ring */
	for (queue = num_ecpri_q; queue < rx_count; queue++) {
		struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
		...
		if (priv->mode == XGMAC_CHAIN_MODE) {
			if (priv->extend_desc)
				xgmac_mode_init(priv, rx_q->dma_erx,
						rx_q->dma_rx_phy, DMA_RX_SIZE, 1);
			else
				xgmac_mode_init(priv, rx_q->dma_rx,
...
err_init_rx_buffers:
	while (queue >= 0) {
		while (--i >= 0)
			xgmac_free_rx_buffer(priv, queue, i);
...
static int init_dma_tx_desc_rings(struct net_device *dev)
{
	...
	for (queue = num_ecpri_q; queue < tx_queue_cnt; queue++) {
		struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
		...
		if (priv->mode == XGMAC_CHAIN_MODE) {
			if (priv->extend_desc)
				xgmac_mode_init(priv, tx_q->dma_etx,
						tx_q->dma_tx_phy, DMA_TX_SIZE, 1);
			else if (!(tx_q->tbs & XGMAC_TBS_AVAIL))
				xgmac_mode_init(priv, tx_q->dma_tx,
 ...
 * @queue: RX queue index
 */
static void dma_free_rx_skbufs(struct xgmac_priv *priv, u32 queue)
{
	int i;
 ...
 * @queue: TX queue index
 */
static void dma_free_tx_skbufs(struct xgmac_priv *priv, u32 queue)
{
	int i;

	for (i = 0; i < DMA_TX_SIZE; i++)
		xgmac_free_tx_buffer(priv, queue, i);
}
...
static void free_dma_tx_desc_resources(struct xgmac_priv *priv)
{
	...
	for (queue = num_ecpri_q; queue < tx_count; queue++) {
		struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
		size_t size;
		void *addr;
...
static void init_pmd_rx_resources(struct xgmac_priv *priv, u32 queue)
{
	struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];

	rx_q->queue_index = queue;
	rx_q->priv_data = priv;
	rx_q->page_pool = NULL;		// Not used
	rx_q->buf_pool = NULL;		// Not used
	rx_q->dma_rx = NULL;		// Not used
	rx_q->dma_rx_phy = 0;		// Not used
}
...
static void init_pmd_tx_resources(struct xgmac_priv *priv, u32 queue)
{
	struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];

	tx_q->queue_index = queue;
	tx_q->priv_data = priv;
	tx_q->tx_skbuff_dma = NULL;	// Not used
	tx_q->tx_skbuff = NULL;		// Not used
	tx_q->dma_tx = NULL;		// Not used
	tx_q->dma_tx_phy = 0;		// Not used
}
...
static int alloc_dma_tx_desc_resources(struct xgmac_priv *priv)
{
	...
	if (priv->plat->ecpri_en) {
		for (queue = 0; queue < num_ecpri_q; queue++) {
			if (ecpri_alloc_tx_resources(priv, queue))
				goto err_dma;
		}
	}
	else if (pmd_enabled) {
		init_pmd_tx_resources(priv, 1);
	}
...
static void xgmac_start_all_dma(struct xgmac_priv *priv)
{
	...
	xgmac_rxp_config(priv, priv->hw->pcsr, priv->tc_entries,
			 priv->tc_entries_max);

	for (chan = 0; chan < rx_channels_count; chan++) {
		/* Disable RPF due to HW issue */
//		xgmac_enable_rpf(priv, priv->ioaddr, 1, chan);
		if (pmd_enabled && chan == 1)
			continue;
...
static void xgmac_dma_operation_mode(struct xgmac_priv *priv)
{
	...
	for (chan = 0; chan < rx_channels_count; chan++) {
		...
		xgmac_dma_rx_mode(priv, priv->ioaddr, rxmode, chan,
				rxfifosz, qmode);
	...
	for (chan = 0; chan < tx_channels_count; chan++) {
		...
		xgmac_dma_tx_mode(priv, priv->ioaddr, txmode, chan,
				txfifosz, qmode);
	}
}
 ...
 */
static int xgmac_tx_clean(struct xgmac_priv *priv, int budget, u32 queue)
{
	struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
	...
	if (unlikely(netif_tx_queue_stopped(netdev_get_tx_queue(priv->dev,
								queue))) &&
	    xgmac_tx_avail(priv, queue) > XGMAC_TX_THRESH) {
	...
	if (tx_q->dirty_tx != tx_q->cur_tx)
		mod_timer(&tx_q->txtimer, XGMAC_COAL_TIMER(priv->tx_coal_timer));
...
{
	struct xgmac_tx_queue *tx_q = &priv->tx_queue[chan];
					  ...
					  u32 rxmode, u32 chan)
{
	...
	xgmac_dma_rx_mode(priv, priv->ioaddr, rxmode, chan, rxfifosz, rxqmode);
	xgmac_dma_tx_mode(priv, priv->ioaddr, txmode, chan, txfifosz, txqmode);
}
#endif
...
static int xgmac_init_dma_engine(struct xgmac_priv *priv)
{
	...
	struct xgmac_rx_queue *rx_q;
	struct xgmac_tx_queue *tx_q;
	u32 chan = 0;
	int atds = 0;
	int ret = 0;
	...
	if (priv->extend_desc && (priv->mode == XGMAC_RING_MODE))
		atds = 1;
...
static void xgmac_tx_timer_arm(struct xgmac_priv *priv, u32 queue)
{
	struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
...
{
	struct xgmac_tx_queue *tx_q = from_timer(tx_q, t, txtimer);
	struct xgmac_priv *priv = tx_q->priv_data;
...
static void xgmac_init_coalesce(struct xgmac_priv *priv)
{
	...
	for (chan = 0; chan < tx_channel_count; chan++) {
		...
		if (pmd_enabled && chan == 1)
			continue;
		timer_setup(&tx_q->txtimer, xgmac_tx_timer, 0);
	}
}
 ...
 */
static void pmd_rxqueues_routing(struct xgmac_priv *priv, bool en)
{
	struct xgmac_flow_entry *entry = &priv->flow_entries[0];

	if (en) {
		entry->in_use = true;
		entry->is_l4 = true;
		entry->ip_proto = IPPROTO_UDP;
		netdev_info(priv->dev, "Reserve L4 filter %d for EGTPU\n",
			    entry->idx);
	} else if (entry->in_use) {
		entry->in_use = false;
		entry->is_l4 = false;
		netdev_info(priv->dev, "Release L4 filter %d for EGTPU\n",
			    entry->idx);
	}
}
...
static void xgmac_mac_config_rx_queues_routing(struct xgmac_priv *priv)
{
	...
	if (pmd_enabled) {
		pmd_rxqueues_routing(priv, true);
		return;
	}
	else {
		pmd_rxqueues_routing(priv, false);
	}
...
static int xgmac_hw_setup(struct net_device *dev, bool init_ptp)
{
...
#if 0 // e56 is configured by u-boot
	if (priv->plat->eth_link == 1)
		xgbe_e56_serdes_config(priv);
#endif
	...
	for (chan = 0; chan < tx_cnt; chan++) {
		struct xgmac_tx_queue *tx_q = &priv->tx_queue[chan];
		int enable = tx_q->tbs & XGMAC_TBS_AVAIL;
...
static int xgmac_request_irqs(struct net_device *dev)
{
	...
	if (priv->plat->per_ch_irq) {
		rx_queue_cnt = priv->plat->rx_queues_to_use;
...
txirq_error:
	for (; queue > 0; queue--) {
		...
		if (pmd_enabled && q_idx == 1)
			continue;
		ch = &priv->channel[q_idx];
		free_irq(ch->tx_irq, ch);
	}
	for (queue = rx_queue_cnt; queue > 0; queue--) {
		u32 q_idx = queue - 1;
...
static int xgmac_open(struct net_device *dev)
{
	...
	int bfsize = 0;
	u32 chan, rx_count, tx_count;
	int linkup;
	int ret;

	/* Configure real RX and TX queues */
	if (pmd_enabled) {
		rx_count = priv->plat->rx_queues_to_use - 1;
		tx_count = priv->plat->tx_queues_to_use - 1;
		if (rx_count < 1)
			rx_count = 1;
		if (tx_count < 1)
			tx_count = 1;
	...
	netif_set_real_num_rx_queues(dev, rx_count);
	netif_set_real_num_tx_queues(dev, tx_count);
	...
	for (chan = 0; chan < priv->plat->tx_queues_to_use; chan++) {
		...
		tx_q->tbs |= tbs_en ? XGMAC_TBS_AVAIL : 0;
		if (xgmac_enable_tbs(priv, priv->ioaddr, tbs_en, chan))
			tx_q->tbs &= ~XGMAC_TBS_AVAIL;
	}
			       ...
			       struct xgmac_tx_queue *tx_q)
{
	...
	tx_q->cur_tx = XGMAC_GET_ENTRY(tx_q->cur_tx, DMA_TX_SIZE);
	return true;
}
...
static netdev_tx_t xgmac_tso_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (unlikely(xgmac_tx_avail(priv, queue) <
	...
	has_vlan = xgmac_vlan_insert(priv, skb, tx_q);
	...
	xgmac_tso_allocator(priv, des, tmp_pay_len, (nfrags == 0), queue);
	...
	if (unlikely(xgmac_tx_avail(priv, queue) <= (MAX_SKB_FRAGS + 1))) {
	...
	xgmac_set_tx_tail_ptr(priv, priv->ioaddr, tx_q->tx_tail_addr, queue);
	xgmac_tx_timer_arm(priv, queue);
...
static netdev_tx_t xgmac_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* Should be no Tx packet to eCPRI queue from stack */
	if (priv->plat->ecpri_en) {
		if (priv->hw->xlgmac)
			num_ecpri_q = 4;
		else
			num_ecpri_q = 1;
		if (queue < num_ecpri_q)
			queue = num_ecpri_q;
	}

	/* If PMD is enabled then re-route traffic from queue 1 to queue 0 */
	if (pmd_enabled && queue == 1) {
		WARN_ON(queue);
		dev_kfree_skb(skb);
		priv->dev->stats.tx_dropped++;
	...
	if (unlikely(xgmac_tx_avail(priv, queue) < nfrags + 1)) {
		if (!netif_tx_queue_stopped(netdev_get_tx_queue(dev, queue))) {
			netif_tx_stop_queue(netdev_get_tx_queue(priv->dev,
								queue));
			/* This is a hard error, log it. */
	...
	if (unlikely(is_jumbo)) {
		entry = xgmac_jumbo_frm(priv, tx_q, skb, csum_insertion);
...
static inline int xgmac_rx_threshold_count(struct xgmac_rx_queue *rx_q)
{
	if (rx_q->rx_zeroc_thresh < XGMAC_RX_THRESH)
		return 0;
 ...
 */
static inline void xgmac_rx_refill(struct xgmac_priv *priv, u32 queue)
{
	struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	int len, dirty = xgmac_rx_dirty(priv, queue);
	unsigned int entry = rx_q->dirty_rx;

	len = DIV_ROUND_UP(priv->dma_buf_sz, PAGE_SIZE) * PAGE_SIZE;

	while (dirty-- > 0) {
		struct xgmac_rx_buffer *buf = &rx_q->buf_pool[entry];
		struct dma_desc *p;
		bool use_rx_wd;
		...
		xgmac_set_desc_addr(priv, p, buf->addr);
		xgmac_set_desc_sec_addr(priv, p, buf->sec_addr);
		xgmac_refill_desc3(priv, rx_q, p);
	...
	xgmac_set_rx_tail_ptr(priv, priv->ioaddr, rx_q->rx_tail_addr, queue);
}
 ...
 */
static int xgmac_rx(struct xgmac_priv *priv, int limit, u32 queue)
{
	struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	struct xgmac_channel *ch = &priv->channel[queue];
...
read_again:
		...
		rx_q->cur_rx = XGMAC_GET_ENTRY(rx_q->cur_rx, DMA_RX_SIZE);
...
static irqreturn_t xgmac_interrupt(int irq, void *data)
{
	...
	for (queue = 0; queue < queues_count; queue++) {
		...
		if (mtl_status & CORE_IRQ_MTL_RX_OVERFLOW) {
			if ((priv->plat->ecpri_en && queue == 0) ||
			    (pmd_enabled && queue == 1)) {
				/* Do nothing */
			}
			else {
				xgmac_set_rx_tail_ptr(priv,
						      priv->ioaddr,
						      rx_q->rx_tail_addr,
						      queue);
			}
...
static irqreturn_t xgmac_uio_isr(int irq, void *data)
{
	...
	uio_event_notify(&priv->uio_dev.uinfo);
...
int xgmac_get_ptp_sec(struct net_device *dev, struct ifreq *ifr)
{
	...
	reg_lo = readl(priv->ioaddr + XGMAC_SYSTEM_TIME_SEC);
	reg_hi = readl(priv->ioaddr + XGMAC_SYSTEM_TIME_SEC_HIGH);
	ptp_sec = reg_hi;
	ptp_sec = (ptp_sec << 32) + reg_lo;
	if (copy_to_user(ifr->ifr_data, &ptp_sec, sizeof(u64)))
		return -EFAULT;

	return 0;
}

static int xgmac_set_pmd_support(struct net_device *dev, struct ifreq *ifr)
{
	struct xgmac_priv *priv = netdev_priv(dev);
	int enable;
	...
	if (copy_from_user(&enable, ifr->ifr_data, sizeof(int)))
		return -EFAULT;
	if (enable == pmd_enabled)
		return 0;

	if (priv->plat->ecpri_en) {
		netdev_err(priv->dev, "must disable eCPRI to support PMD\n");
		return -EINVAL;
	}

	if (netif_running(dev)) {
		netdev_err(priv->dev, "must be stopped to change PMD support\n");
		return -EBUSY;
	}
...
{
	if (copy_to_user(ifr->ifr_data, &pmd_enabled, sizeof(int)))
		return -EFAULT;
...
static int xgmac_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	if (cmd == SIOCGPMDSUPPORT)
		...
		return xgmac_set_pmd_support(dev, rq);
...
static int xgmac_rings_status_show(struct seq_file *seq, void *v)
{
	...
	for (queue = num_rx_ecpri_q; queue < rx_count; queue++) {
		struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	...
	for (queue = num_tx_ecpri_q; queue < tx_count; queue++) {
		struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
...
static int xgmac_uio_init(struct xgmac_priv *priv)
{
	struct xgmac_channel *ch;
	struct device_node *np = priv->device->of_node;
	struct uio_info *uinfo = &priv->uio_dev.uinfo;
	struct resource res;
	int err;
	unsigned int queue_index, rx_irq, rx_irq_cpu;

	queue_index = 1;
	ch = &priv->channel[queue_index];
	rx_irq = priv->plat->rx_queues_cfg[queue_index].ch_irq;
	rx_irq_cpu = priv->plat->rx_queues_cfg[queue_index].ch_irq_cpu;
	...
	snprintf(priv->uio_dev.name, sizeof(priv->uio_dev.name) - 1,
		 "XGMAC%u", priv->plat->phy_lane);
	uinfo->name = priv->uio_dev.name;
	...
	snprintf(ch->rx_irq_name, sizeof(ch->rx_irq_name) - 1,
		 "eth%u-uio-rx-%u", priv->plat->phy_lane, queue_index);
		     ...
		     struct xgmac_resources *res)
{
	...
	if (plat_dat->phy_lane != 0 && xgbe_phy_dev_status() == 0) {
		dev_err(device, "PHY not initialized yet, deferring probe\n");
		return -EPROBE_DEFER;
	}
	...
	if (priv->plat->phy_lane == 0 && res->misc_base == NULL) {
		dev_err(priv->device, "Lane0 Ethernet without misc address\n");
		return -ENODEV;
	}
	priv->misc_base = res->misc_base;
	priv->e56_base = res->e56_base;
	if (priv->plat->phy_lane == 0 && res->bss_base == NULL) {
		dev_err(priv->device, "Lane0 Ethernet without BSS address\n");
		return -ENODEV;
	}
	if (res->bss_base != NULL)
		xgbe_set_bss_addr(res->bss_base);
	if (priv->plat->eth_link == 1 && priv->e56_base == NULL) {
	...
	if (priv->dma_cap.number_rx_queues == 4)
	...
	for (queue = 0; queue < maxq; queue++) {
		...
		if (queue < priv->plat->rx_queues_to_use) {
			...
			ch->rx_irq = priv->plat->rx_queues_cfg[queue].ch_irq;
			ch->rx_irq_cpu = priv->plat->rx_queues_cfg[queue].ch_irq_cpu;
		}
		if (queue < priv->plat->tx_queues_to_use) {
			/* Separate NAPI Tx poll function for eCPRI */
			u32 num_ecpri_q = 0;
			if (priv->hw->xlgmac && priv->plat->ecpri_en)
				num_ecpri_q = 4;
			else
				num_ecpri_q = 1;
			if (priv->plat->ecpri_en && queue < num_ecpri_q) {
			...
			ch->tx_irq = priv->plat->tx_queues_cfg[queue].ch_irq;
			ch->tx_irq_cpu = priv->plat->tx_queues_cfg[queue].ch_irq_cpu;
		}
	}
	...
	if (priv->plat->eth_link == 1) {
		...
		if (ret < 0) {
			netdev_err(ndev, "failed to probe e56 device\n");
			goto error_xpcs;
		}
	}
#endif
...
error_serdes_powerup:
	if (pmd_enabled) {
		free_irq(priv->channel[1].rx_irq, &priv->channel[1]);
		uio_unregister_device(&priv->uio_dev.uinfo);
	}
	unregister_netdev(ndev);
error_netdev_register:
	phylink_destroy(priv->phylink);
	xgbe_phy_unload(priv);
error_phy_setup:
error_xpcs:
	for (queue = 0; queue < maxq; queue++) {
		struct xgmac_channel *ch = &priv->channel[queue];
...
/**
 * xgmac_reset_queues_param - reset queue parameters
 * @dev: device pointer
 */
static void xgmac_reset_queues_param(struct xgmac_priv *priv)
{
	u32 rx_cnt = priv->plat->rx_queues_to_use;
	u32 tx_cnt = priv->plat->tx_queues_to_use;
	u32 queue;
	...
	for (queue = num_rx_ecpri_q; queue < rx_cnt; queue++) {
		struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	...
	for (queue = num_tx_ecpri_q; queue < tx_cnt; queue++) {
		struct xgmac_tx_queue *tx_q = &priv->tx_queue[queue];
