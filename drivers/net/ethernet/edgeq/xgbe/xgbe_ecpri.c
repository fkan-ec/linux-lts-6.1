...
void ecpri_setup_config(struct xgmac_priv *priv)
{
	...
	smp_mb();
	ecpri_cs->is_xlgmac = priv->hw->xlgmac;
	smp_mb();
	...
	if (priv->hw->xlgmac) {
		...
		smp_mb();
		ecpri_cs->tx_desc_ring_size = ecpri_cs->rx_desc_ring_size / 4;
		smp_mb();
...
int ecpri_alloc_rx_resources(struct xgmac_priv *priv, u32 queue)
{
	...
	rx_q->dma_rx = (struct dma_desc *)((u8 *)priv->ecpri_base + ECPRI_RX_DESC_OFFSET);
	rx_q->dma_rx_phy = priv->ecpri_phy_addr + ECPRI_RX_DESC_OFFSET;
...
err_alloc:
	...
	netdev_err(priv->dev, "Error allocating skb for eCPRI queue\n");
...
int ecpri_alloc_tx_resources(struct xgmac_priv *priv, u32 queue)
{
	...
	if (priv->hw->xlgmac)
		offset = ECPRI_XLGMAC_TX_DESC_OFFSET +
			 queue * ECPRI_XLGMAC_TX_PER_Q_DESC_SIZE;
	else
		offset = ECPRI_XGMAC_TX_DESC_OFFSET;
	tx_q->dma_tx = (struct dma_desc *)((u8 *)priv->ecpri_base + offset);
	tx_q->dma_tx_phy = priv->ecpri_phy_addr + offset;
...
static inline void ecpri_rx_refill(struct xgmac_priv *priv, u32 queue,
				   u32 end_entry)
{
	struct xgmac_channel *ch = &priv->channel[queue];
	struct dma_desc *h_desc = priv->h_desc;
...
static int ecpri_rx(struct xgmac_priv *priv, int limit, u32 queue)
{
	struct xgmac_rx_queue *rx_q = &priv->rx_queue[queue];
	struct xgmac_channel *ch = &priv->channel[queue];
...
read_again:
		...
		*/
		...
		skb_record_rx_queue(head_skb, queue);
	...
	if (status & rx_not_ls || head_skb) {
		...
		rx_q->state.skb = head_skb;
		rx_q->state.error = error;
		rx_q->state.len = len;
	}

	if (next_entry != priv->h_buf_hdr->tail)
		ecpri_rx_refill(priv, queue, next_entry);
...
static int ecpri_tx_clean(struct xgmac_priv *priv, u32 queue)
{
	...
	/* Update eCPRI packet stats */
	tx_packets = ecpri_cs->tx_pkt_cnt[queue];
	ecpri_cs->tx_pkt_cnt[queue] = 0;
	smp_mb();
	tx_bytes = ecpri_cs->tx_byte_cnt[queue];
	ecpri_cs->tx_byte_cnt[queue] = 0;
	smp_mb();
	tx_dropped = ecpri_cs->tx_drop_cnt[queue];
	ecpri_cs->tx_drop_cnt[queue] = 0;
	smp_mb();
	tx_error = ecpri_cs->tx_err_cnt[queue];
	ecpri_cs->tx_err_cnt[queue] = 0;
	smp_mb();
